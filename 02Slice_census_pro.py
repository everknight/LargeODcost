...
while i < totaln:
	...
	arcpy.FeatureClassToFeatureClass_conversion(input, outputfolder+'\\'+os.path.splitext(os.path.split(outputgdb)[1])[0]+str(nfile%6)+'.gdb', os.path.splitext(os.path.split(input)[1])[0]+"_"+str(i), "{0} IN ({1})".format(ID, str(criteria[1:len(criteria)-1])))
